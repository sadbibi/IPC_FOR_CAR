...
uchar *ImageProcess::Imageprocess(uchar *origPic)
{
    ...
    for (int i=0;i<60;i++)
    {
        ...
        for(int j=0;j<80;j++)
        {
             ...
             if((j==ImageDeal[i].LeftBorder||j==ImageDeal[i].RightBorder)&&i>ImageStatus.OFFLine)
                a[i*80+j]=RED;

            else if(j==ImageDeal[i].Center&&i>ImageStatus.OFFLine)
                a[i*80+j]=BLUE;
            else
...
void ImageProcess::DrawExtensionLine()
{
           ...
           if (ImageDeal[Ysite].IsLeftFind == 'W')  //如果本行左边界没扫到但扫到的是白色，说明本行没有左边界点
           {
             ...
             while (Ysite >= (ImageStatus.OFFLine + 4))  //此时还没扫到顶边
             {
               ...
               if (ImageDeal[Ysite].IsLeftFind == 'T' &&
                   ...
                   ImageDeal[Ysite - 2].LeftBorder < 70)  //如果扫到本行出现了并且本行以上连续三行都有左边界点（左边界在空白上方）
               {
             ...
             DetL =((float)(ImageDeal[FTSite].LeftBorder -ImageDeal[TFSite].LeftBorder)) /((float)(FTSite - TFSite));  //左边界的斜率：列的坐标差/行的坐标差
       ...
       if (ImageStatus.WhiteLine >= 10)
         TFSite = 55;
       if (ExtenRFlag != 'F')
         for (Ysite = 54; Ysite >= (ImageStatus.OFFLine + 4);Ysite--)  //从第五行开始网上扫扫到顶边下面两行
         {
           PicTemp = Pixle[Ysite];  //存当前行
